...
export default function FacultyPage() {
  ...
  const fetchFaculty = async () => {
    ...
    try {
      ...
      setFaculty([])
      setErrorMessage(getApiErrorMessage(error, "Unable to load faculty."))
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleCreateFaculty = async (data) => {
    ...
    try {
      ...
      setErrorMessage(getApiErrorMessage(error, "Unable to save faculty member."))
    } finally {
  ...
  const handleDelete = async (facultyMember) => {
    try {
      await api.delete(`/api/faculty/${facultyMember._id}`)
      ...
      setErrorMessage(getApiErrorMessage(error, "Unable to delete faculty member."))
    }
  }
  ...
  const columns = [
    {
      ...
      render: (f) => (
        <div className="space-y-1">
          <div className="font-medium text-indigo-100">{f.name}</div>
          <div className="text-sm text-slate-300 flex items-center gap-2">
            <Mail className="h-3 w-3 text-indigo-400" /> {f.email}
          </div>
        </div>
      ),
    },
    ...
    },
    {
      ...
      render: (f) => (
        <div className="flex items-center gap-2 text-slate-200">
          <Clock className="h-3 w-3 text-indigo-400" /> {f.maxHoursPerWeek}h
        </div>
      ),
    },
    {
          ...
          {Object.entries(f.availability || {}).map(([day, slots]) =>
            slots.length ? (
              <div key={day} className="text-slate-300">
                <span className="text-indigo-300 font-medium">{day.charAt(0).toUpperCase() + day.slice(1)}:</span>{" "}
      ...
      render: (f) => (
        <div className="flex gap-2">
          <Button
            size="sm"
            variant="outline"
            className="border-indigo-400/30 bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-300 hover:border-indigo-300 hover:text-indigo-200 transition-all duration-300 backdrop-blur-sm"
            onClick={() => {
              setEditingFaculty(f)
              setShowForm(true)
            }}
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 relative overflow-hidden">
        ...
        <div className="relative z-10 p-8 text-center text-slate-300 flex items-center justify-center min-h-screen">
          <div className="space-y-4">
            <div className="w-16 h-16 border-4 border-indigo-400/30 border-t-indigo-400 rounded-full animate-spin mx-auto"></div>
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 relative overflow-hidden">
      <div className="absolute inset-0 overflow-hidden">
        ...
        {/* Floating particles */}
        <div className="absolute top-20 left-20 w-2 h-2 bg-indigo-400/40 rounded-full animate-bounce delay-300"></div>
        <div className="absolute top-40 right-32 w-1 h-1 bg-indigo-400/40 rounded-full animate-bounce delay-700"></div>
        <div className="absolute bottom-32 left-40 w-1.5 h-1.5 bg-indigo-400/40 rounded-full animate-bounce delay-1000"></div>
        ...
        {errorMessage && (
          <Card className="border border-amber-500/30 bg-amber-500/10 backdrop-blur-xl shadow-xl shadow-amber-500/10">
            <CardHeader className="p-6">
              <CardTitle className="text-lg font-semibold text-amber-100">Data source unavailable</CardTitle>
              <CardDescription className="text-amber-100/80">{errorMessage}</CardDescription>
            </CardHeader>
            <CardContent className="px-6 pb-6 flex items-center gap-3">
              <Button
              ...
              <span className="text-sm text-amber-100/70">
                Once MongoDB is connected, the faculty table will load automatically.
              </span>
            </CardContent>
          </Card>
        )}
