...
export default function NotificationsPage() {
  ...
  const handleSubmitNotification = async (e) => {
    ...
    try {
      ...
      setErrorMessage(getApiErrorMessage(error, "Unable to create notification."))
    } finally {
  ...
  const confirmDeleteNotification = async () => {
    ...
    try {
      ...
      setErrorMessage(getApiErrorMessage(error, "Unable to delete notification."))
    } finally {
  ...
  const handleMarkAsRead = async (id) => {
    try {
      ...
      setErrorMessage(getApiErrorMessage(error, "Unable to mark the notification as read."))
    }
  }
  ...
  const handleMarkAllRead = async () => {
    ...
    try {
      ...
      setErrorMessage(getApiErrorMessage(error, "Unable to mark all notifications as read."))
    }
  }
  ...
  const getNotificationColor = (type, isRead) => {
    ...
    switch (type) {
      ...
      case "info":
      default:
        return "bg-indigo-500/10 border-indigo-400/30 border-l-4 border-l-indigo-400"
    }
  }
            ...
            {navigationItems.map((item) => {
              ...
              return (
                <Link key={item.id} to={item.path} onClick={() => setActiveNavItem(item.id)}>
                  ...
                  >
                    <IconComponent
                      className={`w-5 h-5 transition-all duration-300 ${isActive ? "text-indigo-300" : "text-slate-400 group-hover:text-indigo-400"} group-hover:scale-110`}
                    />
      ...
      <main className="relative z-10 flex-1 overflow-auto p-8 space-y-8">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
          ...
          <Button
            ...
            className="bg-gradient-to-r from-indigo-600 to-indigo-600 hover:from-indigo-500 hover:to-indigo-500 text-white shadow-lg shadow-indigo-600/25 hover:shadow-xl hover:shadow-indigo-600/30 transition-all duration-300 px-6 py-3 flex items-center gap-2 border border-indigo-500/30 backdrop-blur-sm hover:scale-105"
          >
        ...
        {showForm && (
          <Card className="bg-slate-800/40 backdrop-blur-xl border border-slate-700/50 shadow-2xl shadow-indigo-500/10">
            <CardHeader className="border-b border-slate-700/50 p-6">
              <CardTitle className="text-xl font-semibold text-indigo-100">Create New Notification</CardTitle>
            ...
            <CardContent className="p-6">
              <form onSubmit={handleSubmitNotification} className="space-y-4">
                ...
                <div>
                  ...
                  <Input value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} required className="mt-1 bg-slate-800/50 border-slate-600/50 focus:border-indigo-500 focus:ring-indigo-500/20 text-slate-200 placeholder-slate-500 backdrop-blur-sm transition-all duration-300 hover:border-slate-500/70" />
                </div>
                <div>
                  <Label className="text-slate-300 font-medium">Message</Label>
                  <Textarea value={formData.message} onChange={(e) => setFormData({ ...formData, message: e.target.value })} required className="mt-1 bg-slate-800/50 border-slate-600/50 focus:border-indigo-500 focus:ring-indigo-500/20 text-slate-200 placeholder-slate-500 backdrop-blur-sm transition-all duration-300 hover:border-slate-500/70" />
                </div>
                <div className="flex gap-3 pt-2">
                  <Button type="submit" disabled={formLoading} className="bg-gradient-to-r from-indigo-600 to-indigo-600 hover:from-indigo-500 hover:to-indigo-500 text-white shadow-lg shadow-indigo-600/25 transition-all duration-300 border border-indigo-500/30 backdrop-blur-sm">{formLoading ? "Sending..." : "Send Notification"}</Button>
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-2">
                <CardTitle className="flex items-center gap-3 text-xl font-semibold text-indigo-100">
                  <div className="p-2 bg-gradient-to-r from-indigo-500/20 to-indigo-500/20 rounded-xl border border-indigo-400/30 backdrop-blur-sm">
