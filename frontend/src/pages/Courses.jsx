...
export default function CoursesPage() {
  ...
  const fetchCourses = async () => {
    try {
      ...
      setErrorMessage(getApiErrorMessage(err, "Unable to load courses."))
      setCourses([])
    } finally {
      setLoading(false)
    }
  ...
  const handleCreateCourse = async (courseData) => {
    try {
      ...
      setErrorMessage(getApiErrorMessage(error, "Unable to create course."))
    } finally {
  ...
  const handleUpdateCourse = async (id, courseData) => {
    try {
      ...
      setErrorMessage(getApiErrorMessage(error, "Unable to update course."))
    } finally {
      setFormLoading(false)
    }
  }
  ...
  const handleDeleteCourse = async (id) => {
    try {
      ...
      setErrorMessage(getApiErrorMessage(error, "Unable to delete course."))
    }
  }
  ...
  const columns = [
    {
      ...
      sortable: true,
      render: (course) => (
        <div className="font-mono font-semibold text-indigo-100 bg-slate-800/50 px-3 py-1 rounded-lg border border-indigo-500/20">
          {course.code}
        </div>
      ),
    },
    ...
    },
    {
      ...
      label: "Credits",
      render: (course) => (
        <Badge className="bg-gradient-to-r from-indigo-500/20 to-indigo-500/20 text-indigo-300 border-indigo-500/30 hover:from-indigo-500/30 hover:to-indigo-500/30 backdrop-blur-sm">
          {course.credits}
        </Badge>
      ),
    },
    ...
    },
    {
      ...
      label: "Year",
      render: (course) => (
        <Badge className="bg-gradient-to-r from-rose-500/20 to-rose-500/20 text-rose-300 border-rose-500/30 hover:from-rose-500/30 hover:to-rose-500/30 backdrop-blur-sm">
          {course.year}
        </Badge>
      ),
    },
      ...
      render: (course) => (
        <div className="flex gap-2">
          <Button
            size="sm"
            variant="outline"
            className="border-indigo-500/30 bg-slate-800/50 hover:bg-indigo-500/10 text-indigo-300 hover:border-indigo-400/50 hover:text-indigo-200 transition-all duration-300 backdrop-blur-sm"
            onClick={() => {
    ...
    return (
      <div className="flex min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 relative overflow-hidden">
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          ...
          <div className="absolute bottom-1/4 right-1/4 w-80 h-80 bg-gradient-to-r from-indigo-500/10 to-indigo-500/10 rounded-full blur-3xl animate-pulse delay-1000"></div>
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 relative overflow-hidden">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        {/* Floating particles */}
        <div className="absolute top-20 left-20 w-2 h-2 bg-indigo-400/60 rounded-full animate-ping"></div>
        <div className="absolute top-40 right-32 w-1 h-1 bg-indigo-400/60 rounded-full animate-ping delay-700"></div>
        <div className="absolute bottom-32 left-40 w-1.5 h-1.5 bg-indigo-400/60 rounded-full animate-ping delay-300"></div>
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-indigo-400">Smart Classroom</p>
              </div>
            </div>
          </div>
                ...
                <Link key={item.id} to={item.path} onClick={() => setActiveNavItem(item.id)}>
                  <div
                    className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-300 group cursor-pointer ${
                      isActive
                        ? "bg-gradient-to-r from-indigo-500/20 to-indigo-500/20 text-indigo-100 shadow-lg shadow-indigo-500/10 border border-indigo-500/20 backdrop-blur-sm"
                  ...
                  >
                    <IconComponent
                      className={`w-5 h-5 transition-all duration-300 ${
                        isActive ? "text-indigo-300 drop-shadow-sm" : "text-slate-400 group-hover:text-slate-200"
        ...
        <div className="p-8 space-y-8">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            ...
            <Button
              ...
              className="bg-gradient-to-r from-indigo-500 to-indigo-600 hover:from-indigo-600 hover:to-indigo-700 text-white shadow-lg shadow-indigo-500/25 hover:shadow-xl hover:shadow-indigo-500/30 transition-all duration-300 px-6 py-3 border border-indigo-500/20 backdrop-blur-sm"
            >
          ...
          {errorMessage && (
            <Card className="border border-amber-500/30 bg-amber-500/10 backdrop-blur-xl shadow-xl shadow-amber-500/10">
              <CardHeader className="p-6">
                <CardTitle className="text-lg font-semibold text-amber-100">Courses are temporarily unavailable</CardTitle>
          ...
          {showForm && (
            <Card className="bg-slate-800/50 backdrop-blur-xl border-slate-700/50 shadow-2xl shadow-indigo-500/5">
              ...
              <div className="flex items-center justify-between">
                <div className="space-y-2">
                  <CardTitle className="flex items-center gap-3 text-xl font-semibold text-slate-100">
                    <div className="p-2 bg-gradient-to-br from-indigo-500/20 to-indigo-500/20 rounded-xl border border-indigo-500/20 backdrop-blur-sm">
